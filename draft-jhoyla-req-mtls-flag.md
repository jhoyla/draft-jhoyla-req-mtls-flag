...
This mechanism is primarily intended to allow web servers to cryptographically
authenticate web crawlers and other trusted sources of automated traffic
without prompting human users.

The traffic generated by these sources represents a significant and ever
increasing fraction of the overall traffic to a given web server.
It is useful for the server to be able to distinguish these sources from one
another and, especially, from human users, as this allows it to better
accommodate the different requirements of each group of clients. For example,
web browsers are much more latency sensitive than web crawlers. On the other
hand, it may be prudent to treat requests from web browsers with more scrutiny
than requests from a well-known web-crawler whose behaviour is relatively
predictable.

This dynamic creates an incentive for attackers to impersonate trusted sources
of automated traffic to the server: to confer onto itself whatever
accommodations are conferred on trusted sources. Authentication of the TLS
client is sufficient to thwart such attacks, and while there are other
approaches to solving this problem (see {{alternative-approaches}}),
authentication at the TLS layer has several desirable features for this use
case. [[TODO: Address issue \#12 and add a reference here.]]

The mechanism specified in this document may be useful for other use cases.
However, it may entail security, privacy, or operational considerations that
are relevant to these use cases but are not directly addressed here.
