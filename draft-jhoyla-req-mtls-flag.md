---
title: "TLS Flag - Request Bot Auth"
category: info
...
This document defines a TLS Flag that allows a TLS client to signal to the
server that it is a bot. This allows the server to then request a client
certificate without risking the poor UI/UX that users can experience when
connecting to a zone that requests a client certificate.
...
client to hint to the server that it is a bot, and that it is willing to authenticate itself using a
client certificate. Sometimes a server does not want to authenticate every client, but
might wish to authenticate a subset of them, i.e. bots. In TLS 1.3 this may be done with
post-handshake authentication, however this adds an extra round trip, and
requires negotiation at the application layer. For further discussion of
alternative ways of identifying bots see {Appendix}.
...
messages in the wild. A client sending the `request_bot_auth` flag in the
`ClientHello` allows the server to request authentication during the initial
handshake only when it receives a hint that the client is a bot and thus is able
to handle a `CertificateRequest` gracefully without prompting a user. This can include sending an empty certificate message, per {{?RFC8446, Section 4.4.2}}, because the client may not be able to provide a certificate that satisfies the server's request. This allows bots to authenticate themselves when
...
The `request_bot_auth` flag is sent by the client in the `ClientHello`
...
Flags extension, and with the `request_bot_auth` flag set. The server MAY
send a `CertificateRequest` message without the `request_bot_auth` flag set
...
`CertificateRequest` with the `request_bot_auth` flag set, even if said
...
A server receiving the `request_bot_auth` flag in a `ClientHello` message
that wishes to accept the flag MUST echo the `request_bot_auth` flag in the
`CertificateRequest` message. This informs the client that the reason for the
`CertificateRequest` was because of the `request_bot_auth` flag and not due
to some other policy. This allows clients to select certificates based on
whether the server accepted the flag or not.

A server MUST NOT set the `request_bot_auth` flag in the `CertificateRequest`
unless it received the flag in the `ClientHello`. A client receiving the
`request_bot_auth` flag that did not set it in the `ClientHello` MUST generate a
...
## Effect on the Security of the TLS Handshake
...
`request_bot_auth` or not.

## Selection of Trust Anchors

A server may chose to use one set of trust anchors to verify bots and another
set of trust anchors to verify other traffic. In this case care must be taken to
ensure that the appropriate trust anchors are selected in each case, otherwise
it might be possible to incorrectly grant a bot access to resources it should
not be able to reach. If this is a risk in a given deployment a potential
mitigation is to ensure that the set of certificates that chain to the bots
trust anchors and those that chain to other sets of trust anchors are disjoint.
...
* Value shall be TBD
* Flag Name shall be request_bot_auth.
...
`request_bot_auth`, and is also client initiated. This solution is
